...
'''
# Methodology Composition Flow

```mermaid
graph TD
    methodology_composing["methodology_composing()"] --> setup_logging[Setup Logging]
    setup_logging --> composer[Create MethodologyComposer]
    composer --> setup_paths[Setup Project Paths]
    setup_paths --> compose_section[MethodologyComposer.compose_section()]
    
    compose_section --> read_model_code[Read Model Code]
    ...
    subgraph "Subsection Detailization"
        ...
        write_subsection_log --> save_subsection_checkpoint[Save Subsection Checkpoint]
    end
    ...
    class read_model_code,read_agent_files,save_final_output io;
```

This diagram shows the flow of the methodology composition process. The process starts with setting up the environment,
followed by iterative structure generation, subsection detailization, fusion, and finalization.
'''
...
async def methodology_composing(research_field: str, instance_id: str):
    ...
    cache_dir = os.path.join(proj_dir, 'cache_1')
    os.makedirs(cache_dir, exist_ok=True)
    
    # Create an agents directory if it doesn't exist
    agent_dir = os.path.join(cache_dir, 'agents')
    os.makedirs(agent_dir, exist_ok=True)
    
    # Create a workplace/project/model directory if it doesn't exist
    ...
    benchmark_path = os.path.join(benchmark_dir, f'{instance_id}.json')
